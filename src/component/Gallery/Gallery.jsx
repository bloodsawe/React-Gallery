...
export const Gallery = (props) => {
    ...
    let toggleModal = ({ target }) => {
        ...
        if (target.getAttribute("data-user-info)) {
            let i = target.getAttribute("data-modal-index");
        setStorage([...storage, img[i]]);
    ...
    if (target.getAttribute("data-img") && target.getAttribute("data-index")) {
        ...
        setUserImg(img[indexImg].userImageURL)
...
return (
    <Fragment>
        <StyledWrap onClick={toggleModal} >
            ...
            {isOpen ? <Modal modalIndex={modalIndex} modalSize={modalSize} modalSrc={modalSrc} userImg={userImg} /> : null}
        </StyledWrap>
        {img.length === 0 ? null : (
